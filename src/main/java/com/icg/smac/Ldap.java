	...
	public boolean authenticate(String username, String password) throws LoginException {
		...
		do {
			...
			try {
				...
				//HelperExtension.Print(ldapServerUrls[currentLdapUrlIndex]);
			...
			for (int i = 0; i < 3; i++) {
				...
				if (a != null) {
					...
					while (enumeration.hasMoreElements()) {
						...
						/*
						 * for(int k=0;k<values.length;k++) { System.out.println(" values ::: "
						 * +values[k]); }
						 */
					...
					/*
					 * for(int k=0;k<domainServersArray.length;k++) {
					 * System.out.println(" domainServersArray ::: " +domainServersArray[k]); }
					 */
